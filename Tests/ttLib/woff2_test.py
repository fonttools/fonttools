...
class WOFF2ReaderTest(unittest.TestCase):
    ...
    def test_table_tags(self):
        tags = set([t for t in self.font.keys()
                    if t not in ('GlyphOrder', 'DSIG')])
    ...
    def test_get_normal_tables(self):
        ...
        specialTags = woff2TransformedTableTags + \
            ('head', 'GlyphOrder', 'DSIG')
...
class WOFF2ReaderTTFTest(WOFF2ReaderTest):
    ...
    def test_reconstruct_loca(self):
        ...
        # 'glyf' needs to be compiled before 'loca'
        self.font.getTableData("glyf")
...
class WOFF2FlavorDataTest(unittest.TestCase):
    ...
    def test_transformedTables_default(self):
        ...
        self.assertEqual(flavorData.transformedTables,
                         set(woff2TransformedTableTags))
...
class WOFF2WriterTest(unittest.TestCase):
    ...
    def setUp(self):
        self.file.seek(0)
        self.writer = WOFF2Writer(
            BytesIO(), self.numTables, self.font.sfntVersion)
    ...
    def test_checksums(self):
        ...
        self.assertEqual(normCheckSumAdjustment,
                         w2font['head'].checkSumAdjustment)
...
class WOFF2GlyfTableTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.glyphOrder = ['.notdef'] + ["glyph%.5d" %
                                        i for i in range(1, font['maxp'].numGlyphs)]
    ...
    def test_reconstruct_glyf_table_incorrect_size(self):
        ...
        with self.assertRaisesRegex(ttLib.TTLibError, msg):
            WOFF2GlyfTable().reconstruct(
                self.transformedGlyfData[:-1], self.font)
...
class WOFF2RoundtripTest(object):
    ...
    def roundtrip(infile):
        infile.seek(0)
        ttFont = ttLib.TTFont(infile, recalcBBoxes=False,
    ...
    def test_roundtrip_all_transforms(self, ttFont):
        ...
        ttFont.flavorData = WOFF2FlavorData(
            transformedTables=["glyf", "loca", "hmtx"])
        tmp = BytesIO()
        ttFont.save(tmp)
        ...
        assert tmp.getvalue() == tmp2.getvalue()
        assert ttFont2.reader.flavorData.transformedTables == {
            "glyf", "loca", "hmtx"}
...
class MainTest(object):
    ...
    def test_compress_ttf_no_glyf_transform(self, tmpdir):
        ...
        assert woff2.main(
            ["compress", "--no-glyf-transform", input_file]) is None
    ...
    def test_recompress_woff2_keeps_flavorData(self, tmpdir):
        ...
        assert new_woff2_font.flavorData.transformedTables == {
            "glyf", "loca", "hmtx"}

...
TTFONT_DATA_DIR = os.path.join(CURR_DIR, "..", "data")
FULL_TTF_TTX = os.path.join(TTFONT_DATA_DIR, "TestTTF-Regular.ttx")
...
class GlyfTableTest(unittest.TestCase):
    ...
    def test_compile_empty_shell(self):
        font = TTFont(sfntVersion="\x00\x01\x00\x00")
        font.importXML(FULL_TTF_TTX)
        ...
        self.assertEqual(font["maxp"].numGlyphs, 0)
        output = BytesIO()
        font.save(output)
        output.seek(0)
        savedFont = TTFont(output, lazy=True)
        self.assertEqual(savedFont.reader.tables["glyf"].length, 1)
        self.assertEqual(list(savedFont["loca"]), [0])
        self.assertEqual(savedFont["maxp"].numGlyphs, len(glyphNames))
        self.assertEqual(savedFont["maxp"].maxPoints, 0)
        self.assertEqual(savedFont["maxp"].maxContours, 0)
        self.assertEqual(savedFont["maxp"].maxCompositePoints, 0)
        self.assertEqual(savedFont["maxp"].maxCompositeContours, 0)
        self.assertEqual(savedFont["maxp"].maxComponentElements, 0)
        self.assertEqual(savedFont["maxp"].maxComponentDepth, 0)
        self.assertEqual(savedFont["head"].xMin, 0)
        self.assertEqual(savedFont["head"].yMin, 0)
        self.assertEqual(savedFont["head"].xMax, 0)
        self.assertEqual(savedFont["head"].yMax, 0)
        self.assertEqual(savedFont["hhea"].minLeftSideBearing, 0)
        self.assertEqual(savedFont["hhea"].minRightSideBearing, 0)
        self.assertEqual(savedFont["hhea"].xMaxExtent, 0)

    def test_empty_shell_xml_roundtrip(self):
        font = TTFont(sfntVersion="\x00\x01\x00\x00")
        font.importXML(FULL_TTF_TTX)
        glyphNames = font.getGlyphOrder()
        font["loca"].set([0])
        ...
        xml = StringIO()
        font.saveXML(xml)
        glyfXML = xml.getvalue().split("<glyf>", 1)[1].split("</glyf>", 1)[0]
        self.assertNotIn("<TTGlyph", glyfXML)
        xml.seek(0)
        restoredFont = TTFont()
        restoredFont.importXML(xml)
        self.assertEqual(restoredFont["glyf"].glyphs, {})
        output = BytesIO()
        restoredFont.save(output)
        output.seek(0)
        savedFont = TTFont(output, lazy=True)
        self.assertEqual(savedFont.reader.tables["glyf"].length, 1)
        self.assertEqual(list(savedFont["loca"]), [0])
    ...
    def test_compile_nonempty_table_checks_glyph_order(self):
        font = TTFont(sfntVersion="\x00\x01\x00\x00")

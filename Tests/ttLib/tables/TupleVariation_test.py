...
class TupleVariationTest(unittest.TestCase):
    ...
    def test_equal(self):
        var1 = TupleVariation({"wght": (0.0, 1.0, 1.0)},
                              [(0, 0), (9, 8), (7, 6)])
        var2 = TupleVariation({"wght": (0.0, 1.0, 1.0)},
                              [(0, 0), (9, 8), (7, 6)])
    ...
    def test_equal_differentAxes(self):
        var1 = TupleVariation({"wght": (0.0, 1.0, 1.0)},
                              [(0, 0), (9, 8), (7, 6)])
        var2 = TupleVariation({"wght": (0.7, 0.8, 0.9)},
                              [(0, 0), (9, 8), (7, 6)])
    ...
    def test_toXML_points(self):
        writer = XMLWriter(BytesIO())
        g = TupleVariation(
            AXES, [(9, 8), None, (7, 6), (0, 0), (-1, -2), None])
        g.toXML(writer, ["wdth", "wght", "opsz"])
        self.assertEqual([
            '<tuple>',
            '<coord axis="wdth" min="0.25" value="0.375" max="0.5"/>',
    ...
    def test_compileCoord(self):
        ...
        self.assertEqual("C0 00 20 00", hexencode(
            var.compileCoord(["wght", "wdth"])))
        self.assertEqual("20 00 C0 00", hexencode(
            var.compileCoord(["wdth", "wght"])))
    ...
    def test_compileIntermediateCoord(self):
        ...
        self.assertEqual("C0 00 19 9A 00 00 26 66", hexencode(
            var.compileIntermediateCoord(["wght", "wdth"])))
        self.assertEqual("19 9A C0 00 26 66 00 00", hexencode(
            var.compileIntermediateCoord(["wdth", "wght"])))
        self.assertEqual(None, var.compileIntermediateCoord(["wght"]))
        self.assertEqual("19 9A 26 66", hexencode(
            var.compileIntermediateCoord(["wdth"])))

    def test_decompileCoord(self):
        decompileCoord = TupleVariation.decompileCoord_
        data = deHexStr("DE AD C0 00 20 00 DE AD")
        self.assertEqual(({"wght": -1.0, "wdth": 0.5}, 6),
                         decompileCoord(["wght", "wdth"], data, 2))
    ...
    def test_decompileCoord_roundTrip(self):
        ...
        self.assertEqual("7F B9 80 35", hexencode(
            var.compileCoord(["wght", "wdth"])))

    def test_compilePoints(self):
        def compilePoints(p): return TupleVariation.compilePoints(
            set(p), numPointsInGlyph=999)
        # all points in glyph
        self.assertEqual("00", hexencode(compilePoints(range(999))))
        ...
        self.assertEqual("06 05 07 01 F7 02 01 F2", hexencode(
            compilePoints([7, 8, 255, 257, 258, 500])))
        self.assertEqual("03 01 07 01 80 01 EC",
                         hexencode(compilePoints([7, 8, 500])))
        self.assertEqual("04 01 07 01 81 BE E7 0C 0F", hexencode(
            compilePoints([7, 8, 0xBEEF, 0xCAFE])))
        self.maxDiff = None
        self.assertEqual("81 2C" +  # 300 points (0x12c) in total
                         # first run, contains 128 points: [0 .. 127]
                         " 7F 00" + (127 * " 01") +
                         # second run, contains 128 points: [128 .. 255]
                         " 7F" + (128 * " 01") +
                         # third run, contains 44 points: [256 .. 299]
                         " 2B" + (44 * " 01"),
        ...
        self.assertEqual("81 8F" +  # 399 points (0x18f) in total
                         ...
                         # second run, contains 128 points: [128 .. 255]
                         " 7F" + (128 * " 01") +
                         # third run, contains 128 points: [256 .. 383]
                         " 7F" + (128 * " 01") +
                         # fourth run, contains 15 points: [384 .. 398]
                         " 0E" + (15 * " 01"),
    ...
    def test_decompilePoints(self):
        ...
        self.assertEqual(([0xBEEF, 0xCAFE], 6),
                         decompilePoints("02 81 BE EF 0C 0F", 0))
        # 1 point; first run: [7]
        ...
        self.assertEqual(([7, 8, 263, 265], 7),
                         decompilePoints("04 01 07 01 01 FF 02", 0))
        ...
        self.assertEqual(([7, 8, 0xBEEF, 0xCAFE], 13),
                         decompilePoints(data, 4))
    ...
    def test_decompilePoints_roundTrip(self):
        # greater than 255, so we also exercise code path for 16-bit encoding
        numPointsInGlyph = 500

        def compile(points): return TupleVariation.compilePoints(
            points, numPointsInGlyph)
        def decompile(data): return set(
    ...
    def test_compileDeltas_points(self):
        ...
        self.assertEqual("02 01 02 04 82", hexencode(
            var.compileDeltas(points)))
    ...
    def test_compileDeltaValues(self):
        def compileDeltaValues(values): return hexencode(
        ...
        self.assertEqual("01 7F 7F 82 01 7F 7F", compileDeltaValues(
            [127, 127, 0, 0, 0, 127, 127]))
        self.assertEqual("01 7F 7F 83 01 7F 7F", compileDeltaValues(
    ...
    def test_decompileDeltas(self):
        ...
        self.assertEqual(([0, 0, 0, 0], 1),
                         decompileDeltas(4, deHexStr("83"), 0))
        # 41 01 02 FF FF = signed 16-bit values (0x40), count = 2 (1 + 0x41 & 0x3F)
        self.assertEqual(([258, -1], 5), decompileDeltas(2,
                                                         deHexStr("41 01 02 FF FF"), 0))
        # 01 81 07 = signed 8-bit values, count = 2 (1 + 0x01 & 0x3F)
        self.assertEqual(
            ([-127, 7], 3), decompileDeltas(2, deHexStr("01 81 07"), 0))
        ...
        self.assertEqual(([0, 0, 0, 0, 258, -127, -128], 11),
                         decompileDeltas(7, data, 4))

    def test_decompileDeltas_roundTrip(self):
        numDeltas = 30
        compile = TupleVariation.compileDeltaValues_
        def decompile(data): return TupleVariation.decompileDeltas_(
            numDeltas, data, 0)[0]
        for i in range(50):

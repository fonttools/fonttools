...
class PSOperators(object):
    ...
    def ps_matrix(self):
        matrix = [ps_real(1.0), ps_integer(0), ps_integer(
            0), ps_real(1.0), ps_integer(0), ps_integer(0)]
    ...
    def ps_put(self):
        ...
        if tp == 'arraytype' or tp == 'proceduretype':
            ...
            index = obj2.value
            obj3.value = obj3.value[:index] + \
                chr(obj1.value) + obj3.value[index+1:]

    def ps_get(self):
        obj1 = self.pop()
        if obj1.value == "Encoding":
            pass
        obj2 = self.pop('arraytype', 'dicttype', 'stringtype',
                        'proceduretype', 'fonttype')
        tp = obj2.type

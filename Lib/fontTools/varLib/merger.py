...
def _merge_GlyphOrders(font, lst, values_lst=None, default=None):
    ...
    if values_lst is None:
        ...
        dict_sets = [{g: v for g, v in zip(l, vs)}
                     for l, vs in zip(lst, values_lst)]
    combined = set()
...
def merge(merger, self, lst):
    ...
    if not (len(lst) == 1 or (valueFormat & ~0xF == 0)):
        raise VarLibMergeError(
            f"SinglePos format {valueFormat} is unsupported.")
    ...
    self.ValueFormat = reduce(
        int.__or__, [v.getFormat() for v in self.Value], 0)
...
def merge(merger, self, lst):
    ...
    for glyph in glyphs:
        ...
        pvr.Value1 = otBase.ValueRecord(
            merger.valueFormat1) if merger.valueFormat1 else None
        pvr.Value2 = otBase.ValueRecord(
            merger.valueFormat2) if merger.valueFormat2 else None
        pvrs.append(pvr)
        ...
        for j, glyph in enumerate(glyphs):
            ...
            if values[j] is not None:
                ...
                vpair = _Lookup_PairPos_get_effective_value_pair(
                    merger.lookup_subtables[i], self._firstGlyph, glyph)
            if vpair is None:
                v1, v2 = None, None
            else:
                v1 = getattr(vpair, "Value1", None)
                v2 = getattr(vpair, "Value2", None)
            v.Value1 = otBase.ValueRecord(
                merger.valueFormat1, src=v1) if merger.valueFormat1 else None
            v.Value2 = otBase.ValueRecord(
                merger.valueFormat2, src=v2) if merger.valueFormat2 else None
            values[j] = v
                    ...
                    for _ in range(l.Class2Count):
                        if transparent:
                            ...
                            rec2.Value1 = otBase.ValueRecord(
                                self.ValueFormat1) if self.ValueFormat1 else None
                            rec2.Value2 = otBase.ValueRecord(
                                self.ValueFormat2) if self.ValueFormat2 else None
...
def merge(merger, self, lst):
    ...
    self.Coordinate = otRound(
        model.interpolateFromMastersAndScalars(Coords, scalars))
...
def merge(merger, self, lst):
    ...
    self.XCoordinate = otRound(
        model.interpolateFromMastersAndScalars(XCoords, scalars))
    self.YCoordinate = otRound(
        model.interpolateFromMastersAndScalars(YCoords, scalars))
...
def merge(merger, self, lst):
    ...
    for name, tableName in [('XAdvance', 'XAdvDevice'),
        ...
        if hasattr(self, name):
            ...
            value = otRound(
                model.interpolateFromMastersAndScalars(values, scalars))

...
def _add_fvar(font, axes, instances):
    ...
    for instance in instances:
        ...
        inst.coordinates = {axes[k].tag: axes[k].map_backward(
            v) for k, v in coordinates.items()}
...
def _merge_TTHinting(font, masterModel, master_ttfs, tolerance=0.5):
    ...
    for name, glyph in font["glyf"].glyphs.items():
        ...
        if any(pgm != font_pgm for pgm in all_pgms if pgm):
            log.warning(
                "Masters have incompatible glyph programs in glyph '%s', hinting is discarded." % name)
...
def _get_advance_metrics(font, masterModel, master_ttfs,
    ...
    for glyph in glyphOrder:
        vhAdvances = [metrics[glyph][0]
                      if glyph in metrics else None for metrics in advMetricses]
        vhAdvanceDeltasAndSupports[glyph] = masterModel.getDeltasAndSupports(
            vhAdvances)

    singleModel = models.allEqual(id(v[1])
                                  for v in vhAdvanceDeltasAndSupports.values())
...
def load_designspace(designspace):
    ...
    normalized_master_locs = [models.normalizeLocation(
        m, internal_axis_supports) for m in internal_master_locs]

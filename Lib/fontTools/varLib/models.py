    ...
    def getMasterLocationsSortKeyFunc(locations, axisOrder=[]):
        ...
        def getKey(axisPoints, axisOrder):
            ...
            def key(loc):
                ...
                orderedAxes.extend([axis for axis in sorted(
                    loc.keys()) if axis not in axisOrder])
                return (
                    ...
                    tuple(axisOrder.index(
                        axis) if axis in axisOrder else 0x10000 for axis in orderedAxes),
                    tuple(orderedAxes),  # Next, by all axes
                    # Next, by signs of axis values
                    tuple(sign(loc[axis]) for axis in orderedAxes),
                    # Next, by absolute value of axis values
                    tuple(abs(loc[axis]) for axis in orderedAxes),
                )
            return key
...
def main(args=None):
    ...
    if args.designspace:
        ...
        locs = [dict(zip(axes, (float(v) for v in s.split(','))))

...
def cumSum(f, op=add, start=0, decreasing=False):
    ...
    if decreasing:
        def missing(x): return start if x > maxx else total
        domain = range(maxx, minx - 1, -1)
    else:
        def missing(x): return start if x < minx else total
...
def optimizeWidths(widths):
    ...
    dfltCostU = missingdict(lambda x: max(
        cumMaxU[x], cumMaxU[x-108]*2, cumMaxU[x-1132]*5))
    dfltCostD = missingdict(lambda x: max(
        cumMaxD[x], cumMaxD[x+108]*2, cumMaxD[x+1132]*5))

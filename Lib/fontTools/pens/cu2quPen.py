...
OptionalPoints = Sequence[OptionalPoint]
PointsList = Sequence[Points]
OptionalPointsList = Sequence[OptionalPoints]
PenArgs = tuple[Point, ...]
OptionalPenArgs = tuple[OptionalPoint, ...]
MultiPenArgs = Sequence[PenArgs]
OptionalMultiPenArgs = Sequence[OptionalPenArgs]
Stats = MutableMapping[str, int] | None


class Cu2QuPen(FilterPen):
    """A filter pen to convert cubic bezier curves to quadratic b-splines
    using the FontTools SegmentPointPen protocol.

    Args:

        other_pen: another SegmentPointPen used to draw the transformed outline.
    ...
    def _convert_curve(self, pt1: Point, pt2: Point, pt3: Point) -> None:
...
class Cu2QuPointPen(BasePointToSegmentPen):
    ...
    def __init__(
        ...
        if reverse_direction:
            self.pen: AbstractPointPen = ReverseContourPointPen(other_point_pen)
        else:
    ...
    def _flushContour(self, segments: SegmentList) -> None:
        ...
        for segment_type, points in segments:
            if segment_type == "curve":
                for sub_points in self._split_super_bezier_segments(
                    cast(ResolvedSegmentPointList, points)
                ):
        ...
        if closed:
            # the BasePointToSegmentPointPen.endPath method that calls _flushContour
    ...
    def _split_super_bezier_segments(
        self, points: ResolvedSegmentPointList
    ) -> list[ResolvedSegmentPointList]:
        sub_segments = []
        ...
        if n == 2:
            ...
            for i, sub_points in enumerate(
                ...
                new_segment: ResolvedSegmentPointList = []
    ...
    def _drawPoints(self, segments: SegmentList) -> None:
        pen = self.pen
        pen.beginPath()
        last_offcurves = []
    ...
    def addComponent(self, baseGlyphName: str, transformation: TransformInput) -> None:  # type: ignore[override]
...
class Cu2QuMultiPen:
    ...
    in a interpolation-compatible manner, using the FontTools SegmentPointPen protocol.

    Args:

        other_pens: list of SegmentPointPens used to draw the transformed outlines.
        max_err: maximum approximation error in font units. For optimal results,
            if you know the UPEM of the font, we recommend setting this to a
            value equal, or close to UPEM / 1000.
        reverse_direction: flip the contours' direction but keep starting point.

    This pen does not follow the normal SegmentPointPen protocol. Instead, its
    moveTo/lineTo/qCurveTo/curveTo methods take a list of tuples that are
    arguments that would normally be passed to a SegmentPointPen, one item for
    ...
    def __init__(
        ...
        self.pens: Sequence[AbstractPen] = other_pens
        self.max_err = max_err
        self.start_pts: MultiPenArgs | None = None
        self.current_pts: OptionalMultiPenArgs | None = None
    ...
    def moveTo(self, pts: MultiPenArgs) -> None:
        self._check_contour_is_closed()
        self.start_pts = self.current_pts = pts
        self._add_moveTo()

    def lineTo(self, pts: MultiPenArgs) -> None:
    ...
    def qCurveTo(self, pointsList: OptionalMultiPenArgs) -> None:
        self._check_contour_is_open()
        if len(pointsList[0]) == 1:
            self.lineTo([(pt,) for (pt,) in pointsList if pt is not None])
            return
        self._add_moveTo()
        current_pts = []
    ...
    def curveTo(self, pointsList: MultiPenArgs) -> None:
        self._check_contour_is_open()
    ...
    def addComponent(
        self, glyphName: str, transformations: Sequence[TransformInput]
    ) -> None:
        self._check_contour_is_closed()

...
def parseScriptList(lines, featureMap=None):
    ...
    with lines.between('script table'):
        for line in lines:
            ...
            log.debug("Adding script %s language-system %s",
                      scriptTag, langSysTag)

            langSys = ot.LangSys()
            langSys.LookupOrder = None
            if defaultFeature:
                setReference(mapFeature, featureMap, defaultFeature,
                             setattr, langSys, 'ReqFeatureIndex')
            else:
...
def parsePair(lines, font, _lookupMap=None):
    ...
    if typ in ('left', 'right'):
        ...
        self.Class1Record = [ot.Class1Record()
                             for i in range(self.Class1Count)]
        for rec1 in self.Class1Record:
            rec1.Class2Record = [ot.Class2Record()
...
def makeLigatureRecords(data, coverage, c, classCount):
    ...
    for (glyph, klass, compIdx, compCount), anchor in data.items():
        ...
        if record is None:
            ...
            record.ComponentRecord = [ot.ComponentRecord()
        ...
        assert record.ComponentCount == compCount, (
            glyph, record.ComponentCount, compCount)
...
def parseMarkToSomething(lines, font, c):
    ...
    # Base
    self.classCount = 0 if not baseData else 1 + \
        max(k[1] for k, v in baseData.items())
...
class ContextHelper(object):
    def __init__(self, klassName, Format):
        ...
        if Format == 1:
            ...
            def ChainRuleData(r): return (r.Backtrack, r.Input, r.LookAhead)
...
def parseLookupRecords(items, klassName, lookupMap=None):
    ...
    for item in items:
        ...
        setReference(mapLookup, lookupMap,
                     item[1], setattr, rec, 'LookupListIndex')
        lst.append(rec)
    return lst
    ...
    if typ == 'glyph':
        ...
        for line in lines:
            ...
            while len(line) < 1+c.DataLen:
                line.append('')
            seq = tuple(makeGlyphs(stripSplitComma(i))
                        for i in line[1:1+c.DataLen])
...
class Tokenizer(object):
    ...
    def _next_line(self):
        ...
        if line and not line[-1]:
            log.warning('trailing tab found on line %d: %s' %

...
def _log_glyphs(self, glyphs, font=None):
    ...
    if font:
        ...
        self.info("Glyph IDs:   %s", sorted(
            reverseGlyphMap[g] for g in glyphs))
...
def subset_glyphs(self, s):
    if self.Format == 1:
        ...
        class1_map = [c for c in self.ClassDef1.subset(
            s.glyphs, remap=True) if c < self.Class1Count]
        class2_map = [c for c in self.ClassDef2.subset(
            s.glyphs, remap=True) if c < self.Class2Count]
...
def __subset_classify_context(self):
    ...
    class ContextHelper(object):
        def __init__(self, klass, Format):
            ...
            if Format == 1:
                def Coverage(r): return r.Coverage
                def ChainCoverage(r): return r.Coverage
                def ContextData(r): return (None,)
                def ChainContextData(r): return (None, None, None)
                SetContextData = None
                SetChainContextData = None
                def RuleData(r): return (r.Input,)

                def ChainRuleData(r): return (
                ...
                def Coverage(r): return r.Coverage
                def ChainCoverage(r): return r.Coverage
                def ContextData(r): return (r.ClassDef,)

                def ChainContextData(r): return (r.BacktrackClassDef,
                                                 r.InputClassDef,
                                                 r.LookAheadClassDef)
                ...
                def Coverage(r): return r.Coverage[0]
                def ChainCoverage(r): return r.InputCoverage[0]
                ContextData = None
                ChainContextData = None
                SetContextData = None
                SetChainContextData = None
                def RuleData(r): return r.Coverage

                def ChainRuleData(r): return (r.BacktrackCoverage +
                                              r.InputCoverage +
                                              r.LookAheadCoverage)
                ...
                def ChainSetRuleData(r, d):
                    ...
                    (r.BacktrackGlyphCount, r.InputGlyphCount,
                     r.LookAheadGlyphCount,) = (len(x) for x in d)
            else:
                ...
                for ll in getattr(r, c.LookupRecord):
                    ...
                    if seqi in chaos:
                        ...
                        if seqi == 0:
                            ...
                            pos_glyphs = frozenset(ClassDef.intersect_class(
...
def subset_glyphs(self, s):
    ...
    if self.Format == 1:
        ...
        if not self.Coverage.subset(s.glyphs):
            return False
        ContextData = c.ContextData(self)
        klass_maps = [x.subset(s.glyphs, remap=True)
                      if x else None for x in ContextData]
...
def neuter_lookups(self, lookup_indices):
    ...
    self.ensureDecompiled()
    self.Lookup = [l if i in lookup_indices else None for i,
                   l in enumerate(self.Lookup)]
...
@_add_method(otTables.Script)
def collect_features(self):
    feature_indices = [l.LangSys.collect_features()
...
def subset_glyphs(self, s):
    for strike in self.strikes.values():
        strike.glyphs = {g: strike.glyphs[g]
                         for g in s.glyphs if g in strike.glyphs}
...
def subset_glyphs(self, s):
    ...
    varidx_map = varStore.VarStore_subset_varidxes(
        table.VarStore, used, retainFirstMap=retainAdvMap, advIdxes=advIdxes_)
...
@_add_method(ttLib.getTableClass('COLR'))
def subset_glyphs(self, s):
    self.ColorLayers = {g: self.ColorLayers[g]
                        for g in s.glyphs if g in self.ColorLayers}
...
def prune_pre_subset(self, font, options):
    ...
    if options.obfuscate_names:
        ...
        for n in self.names:
            if n.nameID in [1, 4]:
                n.string = ".\x7f".encode(
                    'utf_16_be') if n.isUnicode() else ".\x7f"
...
class Options(object):
    ...
    _drop_tables_default = ['BASE', 'JSTF', 'DSIG', 'EBDT', 'EBLC',
                            'EBSC', 'SVG', 'PCLT', 'LTSH']
    _drop_tables_default += ['Feat', 'Glat',
                             'Gloc', 'Silf', 'Sill']  # Graphite
    ...
    def parse_opts(self, argv, ignore_unknown=[]):
        ...
        for a in argv:
            ...
            if isinstance(ov, bool):
                ...
                if vv == ['']:
                    vv = []
                vv = [int(x, 0) if len(x) and x[0]
                      in "0123456789" else x for x in vv]
                if op == '=':
                    v = vv
                elif op == '+=':
                    v = ov
    ...
    def _subset_glyphs(self, font):
        ...
        with timer("subset GlyphOrder"):
            ...
            if not self.options.retain_gids:
                glyphOrder = [
                    g for g in glyphOrder if g in self.glyphs_retained]
            else:
                glyphOrder = [g for g in glyphOrder if font.getGlyphID(
                    g) <= self.last_retained_order]
            ...
            if tag == 'OS/2' and self.options.prune_unicode_ranges:
                ...
                if self.options.recalc_max_context:
                    ...
                    if max_context != font[tag].usMaxContext:
                        ...
                        log.info("%s usMaxContext updated: %d",
                                 tag, max_context)
            clazz = ttLib.getTableClass(tag)
...
def main(args=None):
    ...
    if options.verbose:
        ...
        log.info("Input font:% 7d bytes: %s" %
                 (os.path.getsize(fontfile), fontfile))
        log.info("Subset font:% 7d bytes: %s" %
                 (os.path.getsize(outfile), outfile))

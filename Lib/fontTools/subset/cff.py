...
def subset_subroutines(self, subrs, gsubrs):
    ...
    for i in range(1, len(p)):
        if p[i] == 'callsubr':
            ...
            p[i-1] = subrs._used.index(p[i-1] +
                                       subrs._old_bias) - subrs._new_bias
        elif p[i] == 'callgsubr':
            assert isinstance(p[i-1], int)
            p[i-1] = gsubrs._used.index(p[i-1] +
                                        gsubrs._old_bias) - gsubrs._new_bias
...
class _DesubroutinizingT2Decompiler(psCharStrings.SimpleT2Decompiler):
    ...
    def execute(self, charString):
        ...
        for idx, expansion in reversed(charString._patches):
            assert idx >= 2
            assert desubroutinized[idx - 1] in ['callsubr',
                                                'callgsubr'], desubroutinized[idx - 1]
        ...
        if not self.private.in_cff2:
            if 'endchar' in desubroutinized:
                ...
                desubroutinized = desubroutinized[:desubroutinized.index(
                    'endchar') + 1]
            else:

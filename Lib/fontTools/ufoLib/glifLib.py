...
class GlyphSet(_UFOBaseIO):
    ...
    def readGlyph(self, glyphName, glyphObject=None, pointPen=None, validate=None):
        ...
        _readGlyphFromTree(tree, glyphObject, pointPen,
                           formatVersions=formatVersions, validate=validate)
    ...
    def writeGlyph(self, glyphName, glyphObject=None, drawPointsFunc=None, formatVersion=None, validate=None):
        ...
        if fileName is None:
            ...
            fileName = self.glyphNameToFileName(
                glyphName, self._existingFileNames.values())
...
def _writeAdvance(glyphObject, element, validate):
    ...
    if width is not None and height is not None:
        etree.SubElement(element, "advance", OrderedDict(
            [("height", repr(height)), ("width", repr(width))]))
...
def _buildOutlineContourFormat1(pen, contour, validate):
    ...
    if len(contour):
        massaged = _validateAndMassagePointStructures(
            contour, pointAttributesFormat1, openContourOffCurveLeniency=True, validate=validate)
...
def _buildOutlineComponentFormat2(pen, component, identifiers, validate):
    ...
    if identifier is not None:
        ...
        identifiers.add(identifier)
    try:
        pen.addComponent(baseGlyphName, tuple(
            transformation), identifier=identifier)
    except TypeError:
        pen.addComponent(baseGlyphName, tuple(transformation))
    ...
    for index, element in enumerate(contour):
        ...
        for attr in ("x", "y"):
            ...
            if validate and value is None:
                raise GlifLibError(
                    "Required %s attribute is missing in point element." % attr)

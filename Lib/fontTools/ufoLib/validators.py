...
def fontInfoOpenTypeGaspRangeRecordsValidator(value):
    ...
    dictPrototype = dict(rangeMaxPPEM=(int, True),
                         rangeGaspBehavior=(list, True))
    ppemOrder = []
...
def fontInfoOpenTypeNameRecordsValidator(value):
    ...
    if not isinstance(value, list):
        return False
    dictPrototype = dict(nameID=(int, True), platformID=(int, True), encodingID=(
        int, True), languageID=(int, True), string=(str, True))
...
def fontInfoWOFFMetadataVendorValidator(value):
    ...
    Version 3+.
    """
    dictPrototype = {"name": (str, True), "url": (
        str, False), "dir": (str, False), "class": (str, False)}
    if not genericDictValidator(value, dictPrototype):
        return False
...
def fontInfoWOFFMetadataCreditsValidator(value):
    ...
    if not len(value["credits"]):
        return False
    dictPrototype = {"name": (str, True), "url": (str, False), "role": (
        str, False), "dir": (str, False), "class": (str, False)}
...
def fontInfoWOFFMetadataLicenseeValidator(value):
    ...
    Version 3+.
    """
    dictPrototype = {"name": (str, True), "dir": (
        str, False), "class": (str, False)}
    if not genericDictValidator(value, dictPrototype):
        return False
    if "dir" in value and value.get("dir") not in ("ltr", "rtl"):
        return False
...
def fontInfoWOFFMetadataTextValue(value):
    ...
    Version 3+.
    """
    dictPrototype = {"text": (str, True), "language": (
        str, False), "dir": (str, False), "class": (str, False)}
    if not genericDictValidator(value, dictPrototype):
        return False
    if "dir" in value and value.get("dir") not in ("ltr", "rtl"):
...
def fontInfoWOFFMetadataExtensionValidator(value):
    ...
    Version 3+.
    """
    dictPrototype = dict(names=(list, False),
                         items=(list, True), id=(str, False))
    if not genericDictValidator(value, dictPrototype):
        return False
    if "names" in value:
...
def fontInfoWOFFMetadataExtensionItemValidator(value):
    ...
    Version 3+.
    """
    dictPrototype = dict(id=(str, False), names=(
        list, True), values=(list, True))
    if not genericDictValidator(value, dictPrototype):
        return False
    for name in value["names"]:
        if not fontInfoWOFFMetadataExtensionNameValidator(name):
    ...
    for key, value in value.items():
        ...
        if key == "public.glyphOrder":
            ...
            if not isinstance(value, (list, tuple)):
                reason = "expected list or tuple, found %s" % type(
                    value).__name__
                return False, bogusGlyphOrderMessage % reason
            for glyphName in value:
                if not isinstance(glyphName, str):
                    reason = "expected str, found %s" % type(
                        glyphName).__name__
                    return False, bogusGlyphOrderMessage % reason
    return True, None

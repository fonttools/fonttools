...
class CFFFontSet(object):
	...
	def decompile(self, file, otFont):
		...
		if self.major == 1:
			...
			self.topDictIndex = TopDictIndex(file, cff2GetGlyphOrder)
			self.strings = IndexedStrings(file)
			self.GlobalSubrs = GlobalSubrsIndex(file)
			self.topDictIndex.strings = self.strings
			self.topDictIndex.GlobalSubrs = self.GlobalSubrs
		...
		for topDict in self.topDictIndex:
			...
			if not hasattr(topDict, "charset") or topDict.charset is None:
				if not isCFF2:
					...
					# Need charset in order to compile the charstrings.
					# will get rid of it later
					charset = otFont.getGlyphOrder()
					topDict.charset = charset
			if isCFF2:
				if topDict.charset == 0:
					topDict.charset = otFont.getGlyphOrder()
				if hasattr(topDict, "Encoding"):
					del topDict.Encoding
		children = topCompiler.getChildren(strings, isCFF2)
...
class IndexCompiler(object):
	...
	def getDataLength(self):
		if self.items:
			...
			dataLength = 2  # count. For empty IINDEx tables, this is the only entry.
...
class IndexedStringsCompiler(IndexCompiler):
	...
	def getDataLength(self):
		l = super(IndexedStringsCompiler, self).getDataLength()
		return l

	def toFile(self, file):
		super(IndexedStringsCompiler, self).toFile(file)
...
class TopDictIndexCompiler(IndexCompiler):
	...
	def getChildren(self, strings, isCFF2):
		children = []
		for topDict in self.items:
			children.extend(topDict.getChildren(strings, isCFF2))
		return children
...
class TopDictIndex(Index):
	...
	def __init__(self, file=None, cff2GetGlyphOrder = False):
...
if 0:
	class VarStoreConverter(BaseTable):
		...
		def write(self, parent, value):
			return 0  # dummy value: data is written by VarStoreCompiler
...
class TopDictCompiler(DictCompiler):
	...
	def getChildren(self, strings, isCFF2):
		children = []
		if hasattr(self.dictObj, "charset") and self.dictObj.charset and (not isCFF2):
			children.append(CharsetCompiler(strings, self.dictObj.charset, self))
		if hasattr(self.dictObj, "Encoding") and (not isCFF2):

...
def equal(lst):
    ...
    first = next(t)
    assert all(
        item == first for item in t), "Expected all items to be equal: %s" % lst
    return first
...
def mergeObjects(lst):
    ...
    for key in allKeys:
        ...
        if mergeLogic is NotImplemented:
            continue
        value = mergeLogic(getattr(table, key, NotImplemented)
                           for table in lst)
...
ttLib.getTableClass('gasp').mergeMap = lambda self, lst: first(
    lst)  # FIXME? Appears irreconcilable
...
def merge(self, m, tables):
    ...
    for fontIdx, table in enumerate(tables):
        ...
        for subtable in table.tables:
            properties = (subtable.format, subtable.platformID,
                          subtable.platEncID)
...
def merge(self, m, tables):
    ...
    for i, (table, dups) in enumerate(zip(tables, m.duplicateGlyphsPerFont)):
        if not dups:
            continue
        assert (table is not None and table is not NotImplemented), "Have duplicates to resolve for font %d but no GSUB: %s" % (
            i + 1, dups)
        synthFeature = None
        synthLookup = None
...
class Merger(object):
    ...
    def merge(self, fontfiles):
        ...
        allTags = reduce(set.union, (list(font.keys())
    ...
    def mergeObjects(self, returnTable, logic, tables):
        ...
        allKeys = set.union(set(), *(vars(table).keys()
                                     for table in tables if table is not NotImplemented))
        for key in allKeys:
            try:
                mergeLogic = logic[key]
            except KeyError:
    ...
    def _preMerge(self, font):
        ...
        for t in [GSUB, GPOS]:
            ...
            if t.table.LookupList:
                lookupMap = {i: v for i, v in enumerate(
                    t.table.LookupList.Lookup)}
                t.table.LookupList.mapLookups(lookupMap)
                t.table.FeatureList.mapLookups(lookupMap)

            if t.table.FeatureList and t.table.ScriptList:
                featureMap = {i: v for i, v in enumerate(
    ...
    def _postMerge(self, font):
        ...
        for t in [GSUB, GPOS]:
            ...
            if t.table.FeatureList and t.table.ScriptList:
                ...
                featureMap = _GregariousIdentityDict(
                    t.table.FeatureList.FeatureRecord)
                t.table.ScriptList.mapFeatures(featureMap)

                # Record used features.
                featureMap = _AttendanceRecordingIdentityDict(
                    t.table.FeatureList.FeatureRecord)
                t.table.ScriptList.mapFeatures(featureMap)
                usedIndices = featureMap.s

                # Remove unused features
                t.table.FeatureList.FeatureRecord = [f for i, f in enumerate(
                    t.table.FeatureList.FeatureRecord) if i in usedIndices]

                # Map back to indices.
                featureMap = _NonhashableDict(
                    t.table.FeatureList.FeatureRecord)
                t.table.ScriptList.mapFeatures(featureMap)
            ...
            if t.table.LookupList:
                ...
                # Record used lookups.
                lookupMap = _AttendanceRecordingIdentityDict(
                    t.table.LookupList.Lookup)
                t.table.FeatureList.mapLookups(lookupMap)
                t.table.LookupList.mapLookups(lookupMap)
                ...
                t.table.LookupList.Lookup = [l for i, l in enumerate(
                    t.table.LookupList.Lookup) if i in usedIndices]

...
class AFM(object):
	...
	def read(self, path):
		...
		for line in lines:
			...
			if m is None:
				raise AFMError("syntax error in AFM file: " + repr(line))
	...
	def parsechar(self, rest):
		m = charRE.match(rest)
		if m is None:
			raise AFMError("syntax error in AFM file: " + repr(rest))
		things = []
		for fr, to in m.regs[1:]:
			things.append(rest[fr:to])
		charname = things[2]
		del things[2]
		charnum, width, l, b, r, t = [int(x) for x in things]
	...
	def parseattr(self, word, rest):
		if word == "FontBBox":
			l, b, r, t = [int(x) for x in rest.split()]
	...
	def write(self, path, sep='\r'):
		...
		items = [(charinfo[0], (charnum, charinfo[1], charinfo[2]))
				for (charnum,charinfo)
				in list(self._chars.items())]
		...
		items = sorted(self._kerning.items()) # XXX is order important?

...
class WOFF2Writer(SFNTWriter):
    ...
    def _calcMasterChecksum(self):
        ...
        directory_end = sfntDirectorySize + \
            len(self.tables) * sfntDirectoryEntrySize
...
woff2DirectoryEntryMaxSize = woff2FlagsSize + \
    woff2UnknownTagSize + 2 * woff2Base128MaxSize
...
class WOFF2GlyfTable(getTableClass('glyf')):
    ...
    def _decodeTriplets(self, glyph):
        ...
        x = 0
        y = 0
        glyph.coordinates = getTableModule(
            'glyf').GlyphCoordinates.zeros(nPoints)
    ...
    def _encodeTriplets(self, glyph):
        ...
        for i in range(len(coordinates)):
            ...
            if x == 0 and absY < 1280:
                ...
                flags.append(onCurveBit + 10 +
                             ((absX & 0xf00) >> 7) + xSignBit)
                ...
                flags.append(onCurveBit + 84 + 12 * (((absX - 1) & 0x300)
                                                     >> 8) + (((absY - 1) & 0x300) >> 6) + xySignBits)

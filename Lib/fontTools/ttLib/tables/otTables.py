...
class FeatureParams(BaseTable):
	...
	No other use has been made, because there is no way to know how to interpret
...
class SingleSubst(FormatSwitchingBaseTable):
	...
	def postRead(self, rawTable, font):
		...
		if self.Format == 1:
			...
			inputGIDS = [doModulo(igids) for igids in inputGIDS]
			outGIDS = [ glyphID + delta for glyphID in inputGIDS ]
			outGIDS = [doModulo(ogid) for ogid in outGIDS]
			outNames = [font.getGlyphName(glyphID) for glyphID in outGIDS ]
			mapping = dict(zip(input, outNames))
		elif self.Format == 2:
			assert len(input) == rawTable["GlyphCount"], \
					"invalid SingleSubstFormat2 table"
			subst = rawTable["Substitute"]
			mapping = dict(zip(input, subst))
		else:
			assert 0, "unknown format: %s" % self.Format
...
class ClassDef(FormatSwitchingBaseTable):
	...
	def postRead(self, rawTable, font):
		...
		if self.Format == 1:
			...
			classDefs = dict(zip(keyList, classList))
			...
			for rec in records:
				...
				classDefs = dict(zip(keyList, [cls]*lenList))
				classDefs[end] = cls
		else:
			assert 0, "unknown format: %s" % self.Format
...
def _buildClasses():
	...
	for name, table in otData:
		...
		if name not in namespace:
			...
			if sys.version > '3':
				cls = types.new_class(name, (baseClass,), {})
			else:
	...
	for base, alts in _equivalents.items():
		...
		for alt in alts:
			if sys.version > '3':
				namespace[alt] = types.new_class(alt, (base,), {})
			else:
	...
	from fontTools.ttLib.tables.otConverters import buildConverters
	for name, table in otData:
		m = formatPat.match(name)
		if m:

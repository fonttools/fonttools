...
class FeatureParams(BaseTable):
    ...
    def compile(self, writer, font):
        assert featureParamTypes.get(writer['FeatureTag']) == self.__class__, "Wrong FeatureParams type for feature '%s': %s" % (
        ...
        if self.Format == 1:
            ...
            for r in ranges:
                ...
                try:
                    ...
                    warnings.warn(
                        "Coverage table has start glyph ID out of range: %s." % start)
                    continue
                try:
                    ...
                    if end != 'glyph65535':
                        warnings.warn(
                            "Coverage table has end glyph ID out of range: %s." % end)
                ...
                glyphs.extend(glyphOrder[glyphID]
    ...
    def postRead(self, rawTable, font):
        ...
        if self.Format == 1:
            ...
            try:
                ...
                warnings.warn(
                    "ClassDef table has start glyph ID out of range: %s." % start)
                startID = len(glyphOrder)
            endID = startID + len(classList)
            if endID > len(glyphOrder):
                warnings.warn("ClassDef table has entries for out of range glyph IDs: %s,%s." %
                              (start, len(classList)))
                # NOTE: We clobber out-of-range things here.  There are legit uses for those,
                # but none that we have seen in the wild.
                endID = len(glyphOrder)
            ...
            for rec in records:
                ...
                try:
                    ...
                    if end != 'glyph65535':
                        warnings.warn(
                            "ClassDef table has end glyph ID out of range: %s." % end)
                    # NOTE: We clobber out-of-range things here.  There are legit uses for those,
                    # but none that we have seen in the wild.
                    endID = len(glyphOrder)
                for glyphID in range(startID, endID):
    ...
    def preWrite(self, font):
        ...
        if items:
            ...
            if len(ranges) * 3 < glyphCount + 1:
                ...
                format = 1
                rawTable = {
                    "StartGlyph": startGlyphName, "ClassValueArray": classes}
        self.Format = format
        return rawTable
...
def fixLookupOverFlows(ttf, overflowRecord):
    ...
    # If the previous lookup is an extType, look further back. Very unlikely,
    # but possible.
...
def fixSubTableOverFlows(ttf, overflowRecord):
    ...
    if hasattr(subtable, 'ExtSubTable'):
        ...
        newExtSubTableClass = lookupTypes[
            overflowRecord.tableType][subtable.__class__.LookupType]

...
class OTTableReader(object):
    ...
    def copy(self):
        other = self.__class__(self.data, self.localState,
                               self.offset, self.tableTag)
        other.pos = self.pos
        return other
...
class ValueRecord(object):
    ...
    def toXML(self, xmlWriter, font, valueName, attrs=None):
        ...
        # "simple" values
        for mask, name, isDevice, format in valueRecordFormat[:4]:
            if hasattr(self, name):
                simpleItems.append((name, getattr(self, name)))
        deviceItems = []
        # device records
        for mask, name, isDevice, format in valueRecordFormat[4:8]:
            if hasattr(self, name):

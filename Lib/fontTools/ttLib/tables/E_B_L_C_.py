...
class table_E_B_L_C_(DefaultTable.DefaultTable):
    ...
    def decompile(self, data, ttFont):
        ...
        # Save the original data because offsets are from the start of the
        # table.
        origData = data
        ...
        for curStrikeIndex in range(self.numSizes):
            ...
            dummy, data = sstruct.unpack2(
                bitmapSizeTableFormatPart1, data, curTable)
            ...
            dummy, data = sstruct.unpack2(
                bitmapSizeTableFormatPart2, data, curTable)

        for curStrike in self.strikes:
            curTable = curStrike.bitmapSizeTable
            for subtableIndex in range(curTable.numberOfIndexSubTables):
                lowerBound = curTable.indexSubTableArrayOffset + \
                ...
                offsetToIndexSubTable = curTable.indexSubTableArrayOffset + \
                    additionalOffsetToIndexSubtable
                data = origData[offsetToIndexSubTable:]

                tup = struct.unpack(
                    indexSubHeaderFormat, data[:indexSubHeaderSize])
                ...
                indexSubTable = indexFormatClass(
                    data[indexSubHeaderSize:], ttFont)
    ...
    def compile(self, ttFont):
        ...
        for curStrike in self.strikes:
            ...
            dataSize += len(('hori', 'vert')) * \
                sstruct.calcsize(sbitLineMetricsFormat)
        ...
        for curStrike in self.strikes:
            ...
            # additionalOffsetToIndexSubtable.
            sizeOfSubTableArray = curTable.numberOfIndexSubTables * \
            ...
            for indexSubTable in curStrike.indexSubTables:
                indexSubTable.additionalOffsetToIndexSubtable = dataSize - \
            ...
            for i in curStrike.indexSubTables:
                data = struct.pack(
                    indexSubHeaderFormat, i.firstGlyphIndex, i.lastGlyphIndex, i.additionalOffsetToIndexSubtable)
                indexSubTablePairDataList.append(data)
            indexSubTablePairDataList.extend(indexSubTableDataList)
            curTable.indexTablesSize = dataSize - \
                curTable.indexSubTableArrayOffset
    ...
    def fromXML(self, name, attrs, content, ttFont):
        ...
        for element in content:
            ...
            if name == 'sbitLineMetrics':
                ...
                assert direction in (
                    'hori', 'vert'), "SbitLineMetrics direction specified invalid."
                metricObj = SbitLineMetrics()
                ...
                print(
                    "Warning: unknown name '%s' being ignored in BitmapSizeTable." % name)
...
# Important information about the naming scheme. Used for identifying
# subtables.
...
def _createOffsetArrayIndexSubTableMixin(formatStringForDataType):
    ...
    class OffsetArrayIndexSubTableMixin(object):
        ...
        def decompile(self):
            ...
            offsetArray = [struct.unpack(dataFormat, self.data[slice(*loc)])[0]
        ...
        def compile(self, ttFont):
            ...
            # must have all its data lined up consecutively. If not this will
            # fail.
            for curLoc, nxtLoc in zip(self.locations, self.locations[1:]):
                assert curLoc[1] == nxtLoc[
                    0], "Data must be consecutive in indexSubTable offset formats"

            glyphIds = list(map(ttFont.getGlyphID, self.names))
            # Make sure that all ids are sorted strictly increasing.
            assert all(glyphIds[i] < glyphIds[i + 1]
            ...
            offsets = list(allLocations[0]) + [loc[1]
    ...
    def readMetrics(self, name, attrs, content, ttFont):
        for element in content:
            ...
            if name == 'imageSize':
                ...
                print("Warning: SmallGlyphMetrics being ignored in format %d." %
                      self.indexFormat)
...
class eblc_index_sub_table_2(FixedSizeIndexSubTableMixin, EblcIndexSubTable):
    ...
    def decompile(self):
        ...
        offsets = [self.imageSize * i +
                   self.imageDataOffset for i in range(len(glyphIds) + 1)]
...
class eblc_index_sub_table_4(EblcIndexSubTable):
    ...
    def decompile(self):
        ...
        indexingLocations = zip(indexingOffsets, indexingOffsets[1:])
        glyphArray = [struct.unpack(codeOffsetPairFormat, data[slice(*loc)])
                      for loc in indexingLocations]
    ...
    def compile(self, ttFont):
        ...
        tmp = [struct.pack(codeOffsetPairFormat, *cop)
...
class eblc_index_sub_table_5(FixedSizeIndexSubTableMixin, EblcIndexSubTable):
    ...
    def decompile(self):
        ...
        data = data[4:]
        glyphIds = [struct.unpack(">H", data[2 * i:2 * (i + 1)])[0]

        ...
        for curStrike in self.strikes:
            ...
            for i in curStrike.indexSubTables:
                data = struct.pack(indexSubHeaderFormat, i.firstGlyphIndex,
                                   i.lastGlyphIndex, i.additionalOffsetToIndexSubtable)
...
def _createOffsetArrayIndexSubTableMixin(formatStringForDataType):
    ...
    class OffsetArrayIndexSubTableMixin(object):
        ...
        def decompile(self):
            ...
            offsetArray = [struct.unpack(
                dataFormat, self.data[slice(*loc)])[0] for loc in indexingLocations]
...
class eblc_index_sub_table_2(FixedSizeIndexSubTableMixin, EblcIndexSubTable):
    ...
    def compile(self, ttFont):
        ...
        assert glyphIds == list(range(
            self.firstGlyphIndex, self.lastGlyphIndex+1)), "Format 2 ids must be consecutive."
...
class eblc_index_sub_table_4(EblcIndexSubTable):
    ...
    def decompile(self):
        ...
        indexingLocations = zip(indexingOffsets, indexingOffsets[1:])
        glyphArray = [struct.unpack(
            codeOffsetPairFormat, data[slice(*loc)]) for loc in indexingLocations]

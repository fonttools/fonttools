...
class table__g_l_y_f(DefaultTable.DefaultTable):
    ...
    def compile(self, ttFont):
        ...
        if currentLocation < 0x20000:
            ...
            indices = [i for i, glyphData in enumerate(
                dataList) if len(glyphData) % 2 == 1]
...
class Glyph(object):
    ...
    def decompileComponents(self, data, glyfTable):
        ...
        if haveInstructions:
            ...
            if len(data) >= 4:
                warnings.warn(
                    "too much glyph data at the end of composite glyph: %d excess bytes" % len(data))
    ...
    def decompileCoordinatesRaw(self, nCoordinates, data):
        ...
        if len(data) - (xDataLen + yDataLen) >= 4:
            warnings.warn("too much glyph data: %d excess bytes" %
                          (len(data) - (xDataLen + yDataLen)))
        xCoordinates = struct.unpack(xFormat, data[:xDataLen])
        yCoordinates = struct.unpack(
            yFormat, data[xDataLen:xDataLen + yDataLen])
    ...
    def recalcBounds(self, glyfTable):
        ...
        if len(coords) > 0:
            if 0:
                    ...
                    for j in range(start, end + 1):
                        if not (flags[j] & flagOnCurve):
                            ...
                            if not pointInRect(coords[j], bbox):
                                # Ouch!
                                warnings.warn(
                                    "Outline has curve with implicit extrema.")
                                # Ouch!  Find analytical curve bounds.
                                pthis = coords[j]
                                plast = coords[last]
                                if not (flags[last] & flagOnCurve):
                                    plast = (
                                        (pthis[0] + plast[0]) / 2, (pthis[1] + plast[1]) / 2)
                                pnext = coords[next]
                                if not (flags[next] & flagOnCurve):
                                    pnext = (
                                        (pthis[0] + pnext[0]) / 2, (pthis[1] + pnext[1]) / 2)
                ...
                self.xMin, self.yMin, self.xMax, self.yMax = calcIntBounds(
                    onCurveCoords)
            else:
                self.xMin, self.yMin, self.xMax, self.yMax = calcIntBounds(
                    coords)
            ...
            for compo in self.components:
                ...
                if not hasattr(compo, "transform"):
                    ...
                    if scale_component_offset:
                        # the Apple way: first move, then scale (ie. scale the
                        # component offset)
                        coordinates.translate(move)
...
class GlyphComponent(object):
    ...
    def decompile(self, data, glyfTable):
        ...
        if self.flags & WE_HAVE_A_SCALE:
            ...
            self.transform = [
                [fi2fl(scale, 14), 0], [0, fi2fl(scale, 14)]]  # fixed 2.14
            data = data[2:]
        elif self.flags & WE_HAVE_AN_X_AND_Y_SCALE:
            xscale, yscale = struct.unpack(">hh", data[:4])
            self.transform = [
                [fi2fl(xscale, 14), 0], [0, fi2fl(yscale, 14)]]  # fixed 2.14
            data = data[4:]
    ...
    def toXML(self, writer, ttFont):
        ...
        if not hasattr(self, "firstPt"):
            ...
            attrs = attrs + \
                [("firstPt", self.firstPt), ("secondPt", self.secondPt)]
            ...
            if transform[0][1] or transform[1][0]:
                attrs = attrs + [
                    ("scalex", transform[0][0]), (
                        "scale01", transform[0][1]),
                                ("scale10", transform[1][0]), (

...
# composite designed not to have the component offset scaled (designed for MS)
                        ...
                        if not (flags[j] & flagOnCurve):
                            ...
                            if not pointInRect(coords[j], bbox):
                                ...
                                if not (flags[last] & flagOnCurve):
                                    plast = ((pthis[0]+plast[0])/2,
                                             (pthis[1]+plast[1])/2)
                                pnext = coords[next]
                                if not (flags[next] & flagOnCurve):
                                    pnext = ((pthis[0]+pnext[0])/2,
                                             (pthis[1]+pnext[1])/2)
...
class GlyphComponent(object):
    ...
    def toXML(self, writer, ttFont):
        ...
        if not hasattr(self, "firstPt"):
            ...
            attrs = attrs + [("firstPt", self.firstPt),

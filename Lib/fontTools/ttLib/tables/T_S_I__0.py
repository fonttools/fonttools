...
class table_T_S_I__0(DefaultTable.DefaultTable):
    ...
    def decompile(self, data, ttFont):
        ...
        assert len(data) == 0
        assert indices[-5] == (
            0XFFFE, 0, -1409540300), "bad magic number"  # 0xABFC1F34
    ...
    def compile(self, ttFont):
        ...
        for index, textLength, textOffset in self.indices:
            data = data + \
                struct.pack(tsi0Format, index, textLength, textOffset)
        data = data + \
            struct.pack(tsi0Format, 0XFFFE, 0, -1409540300)  # 0xABFC1F34

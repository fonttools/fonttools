...
class BaseConverter(object):
    ...
    def __init__(self, name, repeat, aux, tableClass=None):
        ...
        self.isCount = name.endswith("Count") or name in [
            'DesignAxisRecordSize', 'ValueRecordSize']
...
class ExtSubTable(LTable, SubTable):
    ...
    def write(self, writer, font, tableDict, value, repeatIndex=None):
        # actually, mere presence of the field flags it as an Ext Subtable writer.
    ...
    def xmlRead(self, attrs, content, font):
        ...
        for eltName, eltAttrs, eltContent in filter(istuple, content):
            if eltName == "CoverageFlags":
                ...
                assert m.TextDirection in {
                    "Horizontal", "Vertical", "Any"}, "unknown TextDirection %s" % m.TextDirection
...
class _NamedTupleConverter(BaseConverter):
    ...
    def __init__(self, name, repeat, aux, tableClass=None):
        ...
        assert all(issubclass(klass, SimpleValue)
                   for klass in self.converterClasses)

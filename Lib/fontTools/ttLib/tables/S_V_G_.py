...
class table_S_V_G_(DefaultTable.DefaultTable):
    ...
    def compileFormat0(self, ttFont):
        version = 0
        # I start the SVGDocIndex right after the header.
        ...
        if (self.colorPalettes is not None) and (self.colorPalettes.numColorParams is not None):
            ...
            for colorPalette in self.colorPalettes.colorPaletteList:
                writer.begintag("colorPalette", [
                                ("uiNameID", str(colorPalette.uiNameID))])
                writer.newline()
                for colorRecord in colorPalette.paletteColors:
                    colorAttributes = [
...
class ColorPalettes(object):
    def __init__(self):
        ...
        # list of name table name ID values that provide UI description of each color palette.
...
class ColorPalette(object):
    def __init__(self):
        # USHORT. name table ID that describes user interface strings associated with this color palette.
        self.uiNameID = None

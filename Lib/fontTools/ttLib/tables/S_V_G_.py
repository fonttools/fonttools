...
class table_S_V_G_(DefaultTable.DefaultTable):
    ...
    def decompile(self, data, ttFont):
        ...
        if self.version == 1:
            ...
            if self.version != 0:
                print("Unknown SVG table version '%s'. Decompiling as version 0." %
                      (self.version))
                    ...
                    for i in range(numColorPalettes):
                        ...
                        for j in range(numColorParams):
                            colorRecord, colorPaletteData = sstruct.unpack2(
                                colorRecord_format_0, data[pos:], ColorRecord())
    ...
    def decompileEntryList(self, data):
        ...
        self.numEntries = numEntries = struct.unpack(
            ">H", data[pos:pos + 2])[0]
        pos += 2
    ...
    def compileFormat0(self, ttFont):
        ...
        for doc, startGlyphID, endGlyphID in self.docList:
            ...
            curOffset += docLength
            entry = struct.pack(
                ">HHLL", startGlyphID, endGlyphID, docOffset, docLength)
        ...
        if self.colorPalettes is None:
            ...
            for colorPalette in self.colorPalettes.colorPaletteList:
                ...
                for colorRecord in colorPalette.paletteColors:
                    data = struct.pack(
                        ">BBBB", colorRecord.red, colorRecord.green, colorRecord.blue, colorRecord.alpha)
                    dataList.append(data)
            palettesData = bytesjoin(dataList)

        header = struct.pack(
            ">HLL", version, offsetToSVGDocIndex, offsetToColorPalettes)
    ...
    def compileFormat1(self, ttFont):
        ...
        dataList = [header]
        docList = []
        curOffset = SVG_format_1Size + \
            doc_index_entry_format_0Size * numEntries
        for doc, startGlyphID, endGlyphID in self.docList:
            docOffset = curOffset
            docBytes = tobytes(doc, encoding="utf_8")
            docLength = len(docBytes)
    ...
    def toXML(self, writer, ttFont):
        ...
        if (self.colorPalettes is not None) and (self.colorPalettes.numColorParams is not None):
            ...
            for colorPalette in self.colorPalettes.colorPaletteList:
                writer.begintag(
                    "colorPalette", [("uiNameID", str(colorPalette.uiNameID))])
                writer.newline()
                for colorRecord in colorPalette.paletteColors:
                    colorAttributes = [
...
class ColorPalettes(object):
    ...
    def __init__(self):
        ...
        self.colorParamUINameIDs = []
            # list of name table name ID values that provide UI description of
            # each color palette.
    ...
    def fromXML(self, name, attrs, content, ttFont):
        ...
        for colorPalette in self.colorPaletteList:
            if len(colorPalette.paletteColors) != self.numColorParams:
                raise ValueError("Number of color records in a colorPalette ('%s') does not match the number of colorParamUINameIDs elements ('%s')." %

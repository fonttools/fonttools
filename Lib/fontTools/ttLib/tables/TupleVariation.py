...
class TupleVariation(object):
    ...
    def __repr__(self):
        axes = ",".join(sorted(["%s=%s" % (name, value)
        ...
        for axis in axisTags:
            ...
            if value is not None:
                ...
                if minValue == defaultMinValue and maxValue == defaultMaxValue:
                    writer.simpletag("coord", axis=axis,
    ...
    def compile(self, axisTags, sharedCoordIndices, sharedPoints):
        ...
        assert all(tag in axisTags for tag in self.axes.keys()
                   ), ("Unknown axis tag found.", self.axes.keys(), axisTags)
        ...
        if sharedPoints == points:
            ...
            auxData = self.compilePoints(
                points, numPointsInGlyph) + self.compileDeltas(points)
            usesSharedPoints = False

        tupleData = struct.pack('>HH', len(
            auxData), flags) + bytesjoin(tupleData)
    ...
    def compilePoints(points, numPointsInGlyph):
        ...
        if numPoints < 0x80:
            ...
            result = [bytechr((numPoints >> 8) | 0x80) +
                      bytechr(numPoints & 0xff)]
    ...
    def decompilePoints_(numPoints, data, offset, tableTag):
        ...
        if (numPointsInData & POINTS_ARE_WORDS) != 0:
            numPointsInData = (
                numPointsInData & POINT_RUN_COUNT_MASK) << 8 | byteord(data[pos])
            pos += 1
    ...
    def encodeDeltaRunAsWords_(deltas, offset, stream):
        ...
        while pos < numDeltas and runLength < 64:
            ...
            def isByteEncodable(value): return value >= -128 and value <= 127
...
def compileTupleVariationStore(variations, pointCount,
    ...
    if someTuplesSharePoints:
        ...
        data = sharedPointVariation.compilePoints(usedPoints, len(
            sharedPointVariation.coordinates)) + bytesjoin(data)

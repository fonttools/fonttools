...
class Strike(object):
    ...
    def decompile(self, ttFont):
        ...
        sstruct.unpack(sbixStrikeHeaderFormat,
                       self.data[:sbixStrikeHeaderFormatSize], self)
        ...
        self.numGlyphs = (
            firstGlyphDataOffset - sbixStrikeHeaderFormatSize) // sbixGlyphDataOffsetFormatSize - 1
        ...
        for i in range(self.numGlyphs + 1):  # + 1 because there's one more offset than glyphs
            start = i * sbixGlyphDataOffsetFormatSize + \
        ...
        # iterate through offset list and slice raw data into glyph data
        # records
        for i in range(self.numGlyphs):
            current_glyph = Glyph(
                rawdata=self.data[self.glyphDataOffsets[i]:self.glyphDataOffsets[i + 1]], gid=i)
    ...
    def compile(self, ttFont):
        ...
        currentGlyphDataOffset = sbixStrikeHeaderFormatSize + \
        ...
        # add last "offset", really the end address of the last glyph data
        # record
        dummy = Glyph()
    ...
    def toXML(self, xmlWriter, ttFont):
        ...
        for i in range(len(glyphOrder)):
            if glyphOrder[i] in self.glyphs:
                ...
                # TODO: what if there are more glyph data records than (glyf
                # table) glyphs?

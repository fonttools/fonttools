...
class Glyph(object):
    ...
    def decompile(self, ttFont):
        ...
        if len(self.rawdata) > 0:
            if len(self.rawdata) < sbixGlyphHeaderFormatSize:
                from fontTools import ttLib
                # print "Glyph %i header too short: Expected %x, got %x." %
                # (self.gid, sbixGlyphHeaderFormatSize, len(self.rawdata))
    ...
    def compile(self, ttFont):
        ...
        if self.graphicType is None:
            ...
            self.rawdata = sstruct.pack(
                sbixGlyphHeaderFormat, self) + self.imageData

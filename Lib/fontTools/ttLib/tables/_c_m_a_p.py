        ...
        for table in self.tables:
            try:
                ...
                if chunk in done:
                    ...
                    offset = seen[id(
                        table.cmap)] = done[chunk] = totalOffset + len(tableData)
...
class cmap_format_0(CmapSubtable):
    ...
    def compile(self, ttFont):
        ...
        valueList = [getGlyphID(
            cmap[i]) if i in cmap else 0 for i in range(256)]
    ...
    def compile(self, ttFont):
        ...
        for index in range(subheadRangeLen):
            ...
            if subHeader.idRangeOffset == 0:  # didn't find one.
                ...
                # one less subheader, one more subArray.
        ...
        for subhead in subHeaderList:
            dataList.append(struct.pack(subHeaderFormat, subhead.firstCode,
                                        subhead.entryCount, subhead.idDelta, subhead.idRangeOffset))
        for subhead in subHeaderList[:-1]:
...
class cmap_format_12_or_13(CmapSubtable):
    ...
    def compile(self, ttFont):
        ...
        dataList.append(struct.pack(">LLL", startCharCode,
                                    lastCharCode, startGlyphID))
        nGroups = nGroups + 1
        data = bytesjoin(dataList)
...
class cmap_format_14(CmapSubtable):
    ...
    def toXML(self, writer, ttFont):
        ...
        for uvs in uvsList:
            uvList = uvsDict[uvs]
            uvList.sort(key=lambda item: (
                item[1] is not None, item[0], item[1]))
    ...
    def compile(self, ttFont):
        ...
        for uvs in uvsList:
            ...
            vrec = struct.pack(">3sLL", cvtFromUVS(
                uvs), defOVSOffset, nonDefUVSOffset)

...
class table__c_m_a_p(DefaultTable.DefaultTable):
    ...
    def decompile(self, data, ttFont):
        ...
        for i in range(numSubTables):
            ...
            if not length:
                print("Error: cmap subtable is reported as having zero length: platformID %s, platEncID %s,  format %s offset %s. Skipping table." %
    ...
    def compile(self, ttFont):
        self.tables.sort()
                         # sort according to the spec; see
                         # CmapSubtable.__lt__()
        ...
        tableData = b""
        seen = {}
            # Some tables are the same object reference. Don't compile them
            # twice.
        done = {}
            # Some tables are different objects, but compile to the same data
            # chunk
            ...
            try:
                ...
                if chunk in done:
                    ...
                    offset = seen[id(table.cmap)] = done[
                        chunk] = totalOffset + len(tableData)
                    tableData = tableData + chunk
            data = data + \
                struct.pack(">HHl", table.platformID, table.platEncID, offset)
...
class CmapSubtable(object):
    ...
    def __getattr__(self, attr):
        ...
        self.data = None  # Once this table has been decompiled, make sure we don't
                                        ...
                                        # called with an attribute that the
                                        # cmap subtable doesn't have.
...
class cmap_format_0(CmapSubtable):
    ...
    def decompile(self, data, ttFont):
        ...
        # If not, someone is calling  the subtable decompile() directly, and
        # must provide both args.
        if data is not None and ttFont is not None:
            self.decompileHeader(data, ttFont)
        else:
            assert (
                data is None and ttFont is None), "Need both data and ttFont arguments"
    ...
    def compile(self, ttFont):
        ...
        data = struct.pack(
            ">HHH", 0, 262, self.language) + glyphIdArray.tostring()
        assert len(data) == 262
        return data
...
class cmap_format_2(CmapSubtable):
    ...
    def decompile(self, data, ttFont):
        ...
        # value. In this case the final glyph index = 3+ 42 -> 45 for the final
        # glyphIndex. Whew!
    ...
    def compile(self, ttFont):
        ...
        try:
            ...
            try:
                gids = list(
                    map(operator.getitem, [nameMap] * lenCharCodes, names))
            except KeyError:
        ...
        subHeaderKeys = [kEmptyTwoCharCodeRange for x in range(
            256)]  # list of indices into subHeaderList.
        subHeaderList = []
        ...
        # with the result that the subhead 0 would not get created just by
        ...
        for charCode, gid in items:
            ...
            if firstbyte != lastFirstByte:  # Need to update the current subhead, and start a new one.
                ...
                # need to fill in with notdefs all the code points between the
                # last charCode and the current charCode.
                codeDiff = secondByte - \
        ...
        # fix GI's and iDelta of last subheader that we we added to the
        # subheader array.
        self.setIDDelta(subHeader)

        # Now we add a final subheader for the subHeaderKeys which maps to
        # empty two byte charcode ranges.
        subHeader = SubHeader()
        subHeader.firstCode = 0
        subHeader.entryCount = 0
        subHeader.idDelta = 0
        ...
        idRangeOffset = (len(subHeaderList) - 1) * 8 + \
            2  # offset to beginning of glyphIDArray from first subheader idRangeOffset.
        subheadRangeLen = len(subHeaderList) - \
            1  # skip last special empty-set subheader; we've already hardocodes its idRangeOffset to 2.
        for index in range(subheadRangeLen):
            ...
            for j in range(index):
                ...
                if prevSubhead.glyphIndexArray == subHeader.glyphIndexArray:  # use the glyphIndexArray subarray
                    subHeader.idRangeOffset = prevSubhead.idRangeOffset - \
                        (index - j) * 8
                    subHeader.glyphIndexArray = []
                    break
            if subHeader.idRangeOffset == 0:  # didn't find one.
                subHeader.idRangeOffset = idRangeOffset
                idRangeOffset = (idRangeOffset - 8) + subHeader.entryCount * \
                    2  # one less subheader, one more subArray.
            else:
        ...
        for subhead in subHeaderList[:-1]:
            length = length + \
                len(subhead.glyphIndexArray) * \
                    2  # We can't use subhead.entryCount, as some of the subhead may share subArrays.
        ...
        for subhead in subHeaderList:
            dataList.append(
                struct.pack(subHeaderFormat, subhead.firstCode, subhead.entryCount, subhead.idDelta, subhead.idRangeOffset))
        for subhead in subHeaderList[:-1]:
            for gi in subhead.glyphIndexArray:
                dataList.append(struct.pack(">H", gi))
        data = bytesjoin(dataList)
        assert (len(data) == length), "Error: cmap format 2 is not same length as calculated! actual: " + \
            str(len(data)) + " calc : " + str(length)
        return data
...
def splitRange(startCode, endCode, cmap):
    ...
    # to do well with the fonts I tested: none became bigger, many became
    # smaller.
...
class cmap_format_4(CmapSubtable):
        ...
        for i in range(len(startCode) - 1):  # don't do 0xffff!
            ...
            if rangeOffset == 0:
                ...
                for charCode in rangeCharCodes:
                    ...
                    assert (index < lenGIArray), "In format 4 cmap, range (%d), the calculated index (%d) into the glyph index array  is not less than the length of the array (%d) !" % (
                        i, index, lenGIArray)
    ...
    def compile(self, ttFont):
        ...
        if lenCharCodes == 0:
            ...
            charCodes.sort()
            names = list(
                map(operator.getitem, [self.cmap] * lenCharCodes, charCodes))
            nameMap = ttFont.getReverseGlyphMap()
            try:
                gids = list(
                    map(operator.getitem, [nameMap] * lenCharCodes, names))
        ...
        idDelta.append(1)
                       # 0xffff + 1 == (tadaa!) 0. So this end code maps to
                       # .notdef
        idRangeOffset.append(0)
        ...
        if sys.byteorder != "big":
            ...
            restArray.byteswap()
        data = charCodeArray.tostring() + \
            idDeltaArray.tostring(
        ) + restArray.tostring()

        length = struct.calcsize(cmap_format_4_format) + len(data)
        header = struct.pack(
            cmap_format_4_format, self.format, length, self.language,
                        segCountX2, searchRange, entrySelector, rangeShift)
        return header + data
...
class cmap_format_6(CmapSubtable):
    ...
    def decompile(self, data, ttFont):
        ...
        data = data[4:]
        # assert len(data) == 2 * entryCount  # XXX not true in Apple's
        # Helvetica!!!
        ...
        try:
            names = list(
                map(operator.getitem, [glyphOrder] * lenArray, glyphIndexArray))
        except IndexError:
            getGlyphName = self.ttFont.getGlyphName
...
class cmap_format_14(CmapSubtable):
    ...
    def decompile(self, data, ttFont):
        ...
        self.cmap = {}
            # so that clients that expect this to exist in a cmap table won't
            # fail.
        uvsDict = {}
        recOffset = 0
    ...
    def toXML(self, writer, ttFont):
        ...
        for uvs in uvsList:
            ...
            for uv, gname in uvList:
                if gname is None:
                    gname = "None"
                # I use the arg rather than th keyword syntax in order to
                # preserve the attribute order.
                writer.simpletag(
                    "map", [("uvs", hex(uvs)), ("uv", hex(uv)), ("name", gname)])
                writer.newline()
        writer.endtag(self.__class__.__name__)
        writer.newline()
    ...
    def fromXML(self, name, attrs, content, ttFont):
        ...
        if not hasattr(self, "cmap"):
            self.cmap = {}
                # so that clients that expect this to exist in a cmap table
                # won't fail.
    ...
    def compile(self, ttFont):
        ...
        offset = 10 + self.numVarSelectorRecords * \
            11  # current value is end of VarSelectorRecords block.
        data = []
        varSelectorRecords = []
        for uvs in uvsList:

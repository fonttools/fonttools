...
class table_C_P_A_L_(DefaultTable.DefaultTable):
    ...
    def decompile(self, data, ttFont):
        self.version, self.numPaletteEntries, numPalettes, numColorRecords, goffsetFirstColorRecord = struct.unpack(
            ">HHHHL", data[:12])
        assert (self.version <=
                1), "Version of CPAL table is higher than I know how to handle"
        self.palettes = []
        pos = 12
    ...
    def _compilePalette(self, palette):
        ...
        def pack(c): return struct.pack(
            ">BBBB", c.blue, c.green, c.red, c.alpha)
    ...
    def _compileColorRecords(self):
        ...
        for palette in self.palettes:
            ...
            colorRecordIndices.append(struct.pack(
                ">H", index * self.numPaletteEntries))
    ...
    def fromXML(self, name, attrs, content, ttFont):
        if name == "palette":
            ...
            if name == "numPaletteEntries":
                self.paletteEntryLabels = [
                    self.NO_NAME_ID] * self.numPaletteEntries

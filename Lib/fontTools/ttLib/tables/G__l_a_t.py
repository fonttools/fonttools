...
class AttributeRun:
    binaryFormat = (Glat_format_1_entry, Glat_format_23_entry)
...
class Subbox:
    binaryFormat = Glat_format_3_subbox_entry
...
class Octabox:
    binaryFormat = Glat_format_3_octabox_metrics
...
class table_G__l_a_t(DefaultTable.DefaultTable):
    ...
    def decompileAttributes12(self, data, fmt):
        attributes = _Dict()
        while len(data) > 3:
            e, data = sstruct.unpack2(fmt, data, AttributeRun())
    ...
    def decompileAttributes3(self, data):
        if self.hasOctaboxes:
            o, data = sstruct.unpack2(Glat_format_3_octabox_metrics, data, Octabox())
            numsub = bin(o.subboxBitmap).count("1")
            o.subboxes = []
            for b in range(numsub):
                if len(data) >= 8:
                    subbox, data = sstruct.unpack2(
                        Glat_format_3_subbox_entry, data, Subbox()
                    )

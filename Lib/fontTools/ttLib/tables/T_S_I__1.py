...
class table_T_S_I__1(DefaultTable.DefaultTable):
    ...
    extras = {0xfffa: "ppgm", 0xfffb: "cvt",
              0xfffc: "reserved", 0xfffd: "fpgm"}
    ...
    def compile(self, ttFont):
        ...
        for i in range(len(glyphNames)):
            if len(data) % 2:
                data = data + \
                    b"\015"  # align on 2-byte boundaries, fill with return chars. Yum.
            name = glyphNames[i]
        ...
        for i in range(len(codes)):
            if len(data) % 2:
                data = data + \
                    b"\015"  # align on 2-byte boundaries, fill with return chars.
            code, name = codes[i]

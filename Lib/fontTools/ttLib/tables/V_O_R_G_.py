...
class table_V_O_R_G_(DefaultTable.DefaultTable):
    ...
    def decompile(self, data, ttFont):
        ...
        self.majorVersion, self.minorVersion, self.defaultVertOriginY, self.numVertOriginYMetrics = struct.unpack(
            ">HHhH", data[:8])
        assert (self.majorVersion <=
                1), "Major version of VORG table is higher than I know how to handle"
        data = data[8:]
        vids = []
        gids = []
        pos = 0
    ...
    def compile(self, ttFont):
        ...
        header = struct.pack(">HHhH", self.majorVersion, self.minorVersion,
                             self.defaultVertOriginY, self.numVertOriginYMetrics)
    ...
    def toXML(self, writer, ttFont):
        ...
        writer.newline()
        writer.simpletag("numVertOriginYMetrics",
                         value=self.numVertOriginYMetrics)
        writer.newline()
        vOriginTable = []
        ...
        for glyphName in glyphNames:
            try:
                ...
                assert 0, "VORG table contains a glyph name not in ttFont.getGlyphNames(): " + \
                    str(glyphName)
            vOriginTable.append(

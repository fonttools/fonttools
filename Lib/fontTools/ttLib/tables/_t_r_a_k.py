...
class table__t_r_a_k(DefaultTable.DefaultTable):
    ...
    def decompile(self, data, ttFont):
        sstruct.unpack(TRAK_HEADER_FORMAT,
                       data[:TRAK_HEADER_FORMAT_SIZE], self)
        for direction in ('horiz', 'vert'):
            trackData = TrackData()
...
class TrackData(MutableMapping):
    ...
    def compile(self, offset):
        ...
        # sort size values
        sizeDataList = [struct.pack(SIZE_VALUE_FORMAT, fl2fi(sv, 16))
                        for sv in sorted(sizes)]

        data = bytesjoin([trackDataHeader] + entryDataList +
                         sizeDataList + perSizeDataList)
        return data
...
class TrackTableEntry(MutableMapping):
    ...
    def toXML(self, writer, ttFont):
        ...
        for size, perSizeValue in sorted(self.items()):
            writer.simpletag("track", size=fl2str(
                size, 16), value=perSizeValue)
            writer.newline()

...
class table_D_S_I_G_(DefaultTable.DefaultTable):
    ...
    def compile(self, ttFont):
        ...
        headers = [packed]
        offset = len(packed) + self.usNumSigs * \
            sstruct.calcsize(DSIG_SignatureFormat)
        data = []
        for sigrec in self.signatureRecords:
            # first pack signature block
            sigrec.cbSignature = len(sigrec.pkcs7)
            packed = sstruct.pack(
                DSIG_SignatureBlockFormat, sigrec) + sigrec.pkcs7
            data.append(packed)
    ...
    def toXML(self, xmlWriter, ttFont):
        ...
        xmlWriter.newline()
        xmlWriter.simpletag("tableHeader", version=self.ulVersion,
                            numSigs=self.usNumSigs, flag="0x%X" % self.usFlag)
        for sigrec in self.signatureRecords:
            xmlWriter.newline()

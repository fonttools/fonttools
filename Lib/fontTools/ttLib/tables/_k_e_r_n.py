...
class KernTable_format_0(object):
    ...
    def decompile(self, data, ttFont):
        ...
        for k in range(nPairs):
            ...
            kernTable[
                (ttFont.getGlyphName(left), ttFont.getGlyphName(right))] = value
        if len(data):
            warnings.warn(
                "excess data in 'kern' subtable: %d bytes" % len(data))
    ...
    def compile(self, ttFont):
        ...
        kernTable = sorted((getGlyphID(left), getGlyphID(right), value)

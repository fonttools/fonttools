...
class JustifyLevel:
    binaryFormat = Silf_justify_format

    # BEGIN sstruct decode -- generated by MetaTools/buildTableStubs.py
    attrShrink: int
    attrStep: int
    attrStretch: int
    attrWeight: int
...
class PseudoMap:
    binaryFormat = (Silf_pseudomap_format, Silf_pseudomap_format_h)
...
class Silf(object):
    ...
    passes: list["Pass"]
    jLevels: list["JustifyLevel"]
    pMap: dict[int, str]

    # BEGIN sstruct decode -- generated by MetaTools/buildTableStubs.py
    attCollisions: int
    attrBreakWeight: int
    attrDirectionality: int
    attrMirroring: int
    attrPseudo: int
    attrSkipPasses: int
    direction: int
    ...
    def decompile(self, data, ttFont, version=2.0):
        ...
        for jlevel in range(self.numJLevels):
            j, data = sstruct.unpack2(Silf_justify_format, data, JustifyLevel())
        ...
        for i in range(numPseudo):
            if version >= 3.0:
                pseudo = sstruct.unpack(
                    Silf_pseudomap_format, data[8 + 6 * i : 14 + 6 * i], PseudoMap()
                )
            else:
                pseudo = sstruct.unpack(
                    Silf_pseudomap_format_h, data[8 + 4 * i : 12 + 4 * i], PseudoMap()
                )

...
class table__f_v_a_r(DefaultTable.DefaultTable):
    ...
    def compile(self, ttFont):
        header = {
            ...
            "instanceSize": sstruct.calcsize(FVAR_INSTANCE_FORMAT) + (len(self.axes) * 4) + 2
        }
...
class NamedInstance(object):
    ...
    def toXML(self, writer, ttFont):
        ...
        if name is not None:
            ...
            writer.comment(name)
            writer.newline()
        if self.postscriptNameID  == None:
       		writer.begintag("NamedInstance", subfamilyNameID=self.subfamilyNameID)
       	else:
       		writer.begintag("NamedInstance", subfamilyNameID=self.subfamilyNameID, postscriptNameID=self.postscriptNameID, )
        writer.newline()

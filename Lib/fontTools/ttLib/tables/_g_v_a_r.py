...
class table__g_v_a_r(DefaultTable.DefaultTable):
    ...
    def compile(self, ttFont):
        ...
        header["offsetToSharedTuples"] = GVAR_HEADER_SIZE + \
            len(compiledOffsets)
        header["glyphCount"] = len(compiledGlyphs)
        header["flags"] = tableFormat
        header["offsetToGlyphVariationData"] = header["offsetToSharedTuples"] + \
            sharedTupleSize
    ...
    def decompile(self, data, ttFont):
        ...
        offsets = self.decompileOffsets_(data[GVAR_HEADER_SIZE:], tableFormat=(
        ...
        for i in range(self.glyphCount):
            ...
            gvarData = data[offsetToData + offsets[i]
                : offsetToData + offsets[i + 1]]
            try:
...
def compileGlyph_(variations, pointCount, axisTags, sharedCoordIndices):
    ...
    if tupleVariationCount == 0:
        return b""
    result = (
        struct.pack(">HH", tupleVariationCount,
                    4 + len(tuples)) + tuples + data
    )
    if len(result) % 2 != 0:

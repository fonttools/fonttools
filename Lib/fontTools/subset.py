...
def subset(self, glyphs, remap=False):
    ...
    self.classDefs = dict((g, v)
                          for g, v in self.classDefs.items() if g in glyphs)
        ...
        for ll in getattr(r, c.LookupRecord):
            ...
            if seqi in chaos:
                ...
                if seqi == 0:
                    ...
                    pos_glyphs = frozenset(
                        r.InputCoverage[seqi].intersect_glyphs(s.glyphs))
            lookup = s.table.LookupList.Lookup[ll.LookupListIndex]
            chaos.add(seqi)
            if lookup.may_have_non_1to1():
...
def prune_pre_subset(self, options):
    ...
    for st in self.SubTable:
        if not st:
            continue
        if st.prune_pre_subset(options):
            ret = True
    return ret
...
def prune_post_subset(self, options):
    ...
    for st in self.SubTable:
        if not st:
            continue
        if st.prune_post_subset(options):
            ret = True
    return ret
...
def prune_post_subset(self, options):
    ...
    for l in self.Lookup:
        if not l:
            continue
        if l.prune_post_subset(options):
            ret = True
    return ret
...
def subset_glyphs(self, s):
    self.metrics = dict((g, v)
                        for g, v in self.metrics.items() if g in s.glyphs)
...
def subset_subroutines(self, subrs, gsubrs):
    ...
    for i in range(1, len(p)):
        if p[i] == 'callsubr':
            ...
            p[i - 1] = subrs._used.index(
                p[i - 1] + subrs._old_bias) - subrs._new_bias
        elif p[i] == 'callgsubr':
            assert isinstance(p[i - 1], int)
            p[i - 1] = gsubrs._used.index(
                p[i - 1] + gsubrs._old_bias) - gsubrs._new_bias
...
class _DehintingT2Decompiler(psCharStrings.SimpleT2Decompiler):
    ...
    def execute(self, charString):
        old_hints = charString._hints if hasattr(
            charString, '_hints') else None
...
class _DesubroutinizingT2Decompiler(psCharStrings.SimpleT2Decompiler):
    ...
    def execute(self, charString):
        ...
        old_desubroutinized = charString._desubroutinized if hasattr(
            charString, '_desubroutinized') else None
        ...
        for idx, expansion in reversed(charString._patches):
            ...
            assert desubroutinized[idx - 1] in [
                'callsubr', 'callgsubr'], desubroutinized[idx - 1]
...
def prune_post_subset(self, options):
    ...
    for fontname in cff.keys():
        ...
        if hasattr(font, 'FDSelect'):
            all_subrs.extend(fd.Private.Subrs for fd in font.FDArray if hasattr(
                fd.Private, 'Subrs') and fd.Private.Subrs)
...
class Options(object):
    ...
    _drop_tables_default += [
        'Feat', 'Glat', 'Gloc', 'Silf', 'Sill']  # Graphite
    ...
    def parse_opts(self, argv, ignore_unknown=False):
        ...
        for a in argv:
            ...
            if isinstance(ov, bool):
                ...
                if vv == ['']:
                    vv = []
                vv = [int(x, 0) if len(x) and x[
                    0] in "0123456789" else x for x in vv]
                if op == '=':
                    v = vv
                elif op == '+=':
                    v = ov

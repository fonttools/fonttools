...
class TestPlistlib(object):

    def test_io(self, tmpdir, pl):
    ...
    def test_apple_formatting(self):
    ...
    def test_apple_formatting_fromliteral(self, pl):
        pl2 = plistlib.loads(TESTDATA)
        assert pl == pl2

    def test_apple_roundtrips(self):
        pl = plistlib.loads(TESTDATA)
        data = plistlib.dumps(pl)
        pl2 = plistlib.loads(data)
        data2 = plistlib.dumps(pl2)
    ...
    def test_keysort_bytesio(self, sort_keys):
    ...
    @pytest.mark.parametrize("sort_keys", [False, True])
    def test_keysort(self, sort_keys):
        pl = collections.OrderedDict()
        pl['b'] = 1
        pl['a'] = 2
        pl['c'] = 3
    ...
    def test_xml_encodings(self, pl, xml_encoding, encoding, bom):
